	...
	function understrap_posted_on() {
		...
		$posted_on   = apply_filters(
			...
			sprintf(
				...
				esc_html_x( 'Posted on', 'post date', 'alcof' ),
		...
		$byline      = apply_filters(
			...
			sprintf(
				...
				$posted_on ? esc_html_x( 'by', 'post author', 'alcof' ) : esc_html_x( 'Posted by', 'post author', 'alcof' ),
...
if ( ! function_exists( 'understrap_entry_footer' ) ) {
	...
	function understrap_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'alcof' ) );
			if ( $categories_list && understrap_categorized_blog() ) {
				/* translators: %s: Categories of current post */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %s', 'alcof' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'alcof' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %s', 'alcof' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'alcof' ), esc_html__( '1 Comment', 'alcof' ), esc_html__( '% Comments', 'alcof' ) );
			echo '</span>';
		}
		edit_post_link(
			sprintf(
				/* translators: %s: Name of current post */
				esc_html__( 'Edit %s', 'alcof' ),
